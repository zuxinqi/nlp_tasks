...
class Simple_Lexicon_Lstm_Crf_Entity_Extraction(object):
    ...
    def data_process_init(self):
        ...
        :return:
        """
        char_emb = self.dp.config.get('DATA_PROCESS', 'char_emb')
        bichar_emb = self.dp.config.get('DATA_PROCESS', 'bichar_emb')
        gaz_file = self.dp.config.get('DATA_PROCESS', 'gaz_file')
        train_file = self.dp.config.get('DATA_PROCESS', 'train_file')
        dev_file = self.dp.config.get('DATA_PROCESS', 'dev_file')
        test_file = self.dp.config.get('DATA_PROCESS', 'test_file')
        self.dp.init(char_emb, bichar_emb, gaz_file, train_file, dev_file, test_file)
    ...
    def write_param(self,param,acc_dict,f):
        ...
        for i in dir(param):
            if "__" not in i and i not in ["pretrain_word_embedding","pretrain_biword_embedding","pretrain_gaz_embedding"]:
    ...
    def train(self):
        ...
        param = self.dp.param
        param.pretrain_word_embedding = self.dp.data.pretrain_word_embedding
        param.pretrain_biword_embedding = self.dp.data.pretrain_biword_embedding
        param.pretrain_gaz_embedding = self.dp.data.pretrain_gaz_embedding
        param.tag2id = self.dp.data.label_alphabet.instance2index
        param.tag2id["X"] = 0
        param.id2tag = {v: k for k, v in param.tag2id.items()}
        param.num_classes = len(param.tag2id)
        ...
        eu = Evaluation_Utils()
        # 加载模型
        graph,word_inputs,biword_inputs,layer_gaz,gaz_count,gaz_chars,gaz_mask,gazchar_mask,labels,sequence_lengths,dropout_pl,loss,preds,train_op,global_step = make_graph(param)
        # 制作训练集和测试集
        train_seq_data = self.dp.data.train_Ids
        test_seq_data = self.dp.data.test_Ids
        # 训练模型
        with tf.Session(graph=graph) as sess:
            if param.is_training:
                ...
                for epoch in range(param.num_train_epochs):
                    ...
                    all_acc = 0
                    count_loss = 0
                    for word_Ids_, biword_Ids_, label_Ids_, gazs_, gazs_count_, layergazmasks_, gaz_char_Ids_, gazchar_masks_, sentence_lengths_ in self.dp.get_batch(train_seq_data, param.batch_size,param.shuffle):
                        _, l, preds_, global_nums = sess.run(
                            [train_op, loss, preds, global_step], {
                                word_inputs: word_Ids_,
                                biword_inputs: biword_Ids_,
                                layer_gaz: gazs_,
                                gaz_count: gazs_count_,
                                gaz_chars: gaz_char_Ids_,
                                gaz_mask: layergazmasks_,
                                gazchar_mask: gazchar_masks_,
                                labels: label_Ids_,
                                sequence_lengths: sentence_lengths_,
                                dropout_pl: param.dropout_rate
                            })

                        if global_nums % param.display_per_step == 0:
                            # 获取真实序列、标签长度。
                            label_list, pred_list = eu.make_mask(preds_, label_Ids_, sentence_lengths_)
                            all_list = np.concatenate((label_list, pred_list), axis=0)
                            all_list = np.unique(all_list)
                            target_names = [param.id2tag[i] for i in all_list]
                            ...
                            repo_res_dict = eu.calculate_report(label_list, pred_list, param.id2tag)
                        ...
                        if global_nums % param.evaluation_per_step == 0:
                            print('-----------------valudation---------------')
                            word_Ids_, biword_Ids_, label_Ids_, gazs_, gazs_count_, layergazmasks_, gaz_char_Ids_, gazchar_masks_, sentence_lengths_ = next(self.dp.get_batch(test_seq_data, param.batch_size, param.shuffle))
                            l, preds_ = sess.run([loss, preds],
                                                            feed_dict={
                                                                    word_inputs: word_Ids_,
                                                                    biword_inputs: biword_Ids_,
                                                                    layer_gaz: gazs_,
                                ...
                                print(
                                    '{}   Rec:{:6.2%}  Pre:{:6.2%}   f1:{:6.2%}  count:  {}'.format(key,
                                                                                                    ...
                                                                                                    repo_res_dict[key][
                                                                                                        3]))
                            print('-----------------valudation---------------')
                    for word_Ids_, biword_Ids_, label_Ids_, gazs_, gazs_count_, layergazmasks_, gaz_char_Ids_, gazchar_masks_, sentence_lengths_ in self.dp.get_batch(
                ...
                repo_res_dict = eu.calculate_report(all_label_list, all_pred_list, param.id2tag)
                for key in repo_res_dict.keys():
                    print(
    ...
    def make_all_predict(self,param_save_path,optional_reload_path=""):
        ...
        eu = Evaluation_Utils()
        # 加载模型
        graph, word_inputs, biword_inputs, layer_gaz, gaz_count, gaz_chars, gaz_mask, gazchar_mask, labels, sequence_lengths, dropout_pl, loss, preds, train_op, global_step = make_graph(
            param)
        # 制作训练集和测试集
        test_seq_data = self.dp.data.test_Ids
...
if __name__ == '__main__':
    ...
    param_save_path = './param_dir/param_2021_05_14_16_10.pkl'
    # optional_reload_path = ""
    optional_reload_path = "model_12_-_3.2668.ckpt"
    ...
    SLLC = Simple_Lexicon_Lstm_Crf_Entity_Extraction(cfg_path)

    # 初始化数据，开启训练
    # SLLC.data_process_init()
    # SLLC.train()

    # 加载已处理好的数据，开启训练
    # SLLC.data_process_reload()
    # SLLC.train()

    # 根据训练好的模型参数，预测整个测试集
    SLLC.data_process_reload()
    SLLC.make_all_predict(param_save_path,optional_reload_path)
