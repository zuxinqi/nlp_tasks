...
class Biaffine_Bert_Lstm_Crf_Entity_Extraction(object):
    ...
    def train(self):
        ...
        eu = Evaluation_Utils()
        # 加载模型
        graph,word_ids,input_mask,segment_ids,labels,sequence_lengths,dropout_pl,pred,loss,(train_op_bert, train_op_other),global_add = make_graph(param)
        # 制作训练集和测试集
        ...
        with tf.Session(graph=graph) as sess:
            if param.is_training:
                ...
                for epoch in range(param.num_train_epochs):
                    ...
                    all_acc = 0
                    count_loss = 0
                    for seqs_, masks_, segments_, labels_, seq_len_ in self.dp.get_batch(train_seq_data, param.batch_size,
                                                                                 self.dp.tag2id, param.max_seq_length, param.shuffle):
                        _, l, global_nums, p = sess.run(
                            [(train_op_bert, train_op_other), loss, global_add, pred], {
                                word_ids: seqs_,
                                input_mask: masks_,
                                segment_ids: segments_,
                        ...
                        if global_nums % param.evaluation_per_step == 0:
                            print('-----------------valudation---------------')
                            seqs_, masks_, segments_, labels_, seq_len_ = next(self.dp.get_batch(test_seq_data, param.batch_size,
                                                                                         self.dp.tag2id, param.max_seq_length,  param.shuffle))
                                ...
                                print(
                                    '{}   Rec:{:6.2%}  Pre:{:6.2%}   f1:{:6.2%}  count:  {}'.format(key,
                                                                                                    ...
                                                                                                    repo_res_dict[key][
                                                                                                        3]))
                            print('-----------------valudation---------------')
                    for seqs_, masks_, segments_, labels_, seq_len_ in self.dp.get_batch(test_seq_data, param.batch_size,
                                                                                  self.dp.tag2id, param.max_seq_length, param.shuffle):
                        l, p = sess.run([loss, pred],
                                                                  feed_dict={
                                                                      word_ids: seqs_,
    ...
    def make_all_predict(self,param_save_path,optional_reload_path=""):
        ...
        eu = Evaluation_Utils()
        # 加载模型
        graph, word_ids, input_mask, segment_ids, labels, sequence_lengths, dropout_pl, pred, loss, (
        train_op_bert, train_op_other), global_add = make_graph(param)
        # 制作训练集和测试集
        test_seq_data = self.dp.parse_data(self.dp.test_data, self.dp.bert_tokenizer, param.max_seq_length)
    ...
    # 对整个测试集进行测试
    def make_all_predict11(self,param_save_path,optional_reload_path=""):
        """
        对整个训练集进行测试，可改变参数文件
        :param param_save_path: 保存的参数文件路径
        :param optional_reload_path: 自选的加载的参数文件路径，如果为空字符串，则自动选择最低loss模型
        ...
        with tf.Session(graph=graph) as sess:
            ...
            for seqs_, masks_,segments_,labels_, seq_len_ in self.dp.get_batch(test_seq_data, param.batch_size,
                ...
                # 获取真实序列、标签长度。
                label_list, pred_list = eu.make_mask(labels_, p, seq_len_, self.dp.id2tag, param.max_seq_length,action="pred")
                # label_list = [self.dp.BIO_tag2id[i] for i in label_list]
                # pred_list = [self.dp.BIO_tag2id[i] for i in pred_list]
                all_label_list.extend(label_list)
                all_pred_list.extend(pred_list)
                all_ids_list.extend(seqs_.tolist())
                seq_len_list.extend(seq_len_)
                n += 1
                all_loss += l
            with open("bbb.pkl", 'wb') as f:
                import pickle
                pickle.dump([all_label_list,all_pred_list,all_ids_list],f)
            average_loss = all_loss / n
            all_list = np.concatenate((all_label_list, all_pred_list), axis=0)
            all_list = np.unique(all_list)
            target_names = [self.dp.BIO_id2tag[i] for i in all_list]
...
if __name__ == '__main__':
    ...
    param_save_path = './param_dir/param_2021_05_20_10_16.pkl'
    # optional_reload_path = ""
    optional_reload_path = "model_6_-_59.1607.ckpt"
    ...
    BBLC = Biaffine_Bert_Lstm_Crf_Entity_Extraction(cfg_path)

    # 初始化数据，开启训练
    # BBLC.data_process_init()
    # BBLC.train()

    # 加载已处理好的数据，开启训练
    # BBLC.data_process_reload()
    # BBLC.train()

    # 根据训练好的模型参数，预测整个测试集
    BBLC.data_process_reload()
    BBLC.make_all_predict11(param_save_path,optional_reload_path)
