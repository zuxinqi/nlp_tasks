...
class Evaluation_Utils(Base_Evaluation):
    ...
    def remove_cover_index(self,input_index_res):
        input_index_res = input_index_res.tolist()
        covered_list = []
        for i in input_index_res:
            for j in input_index_res:
                if i != j:
                    if i[0] == j[0]:
                        # 有覆盖情况的话，把长的去掉
                        ...
                        # 有覆盖情况的话，把短的去掉
                        if i[1] >= j[1] and i[2] <= j[2]:
                            if i not in covered_list:
                                covered_list.append(i)
        for k in covered_list:
            input_index_res.remove(k)
        return input_index_res


    def make_tags(self, index_res, labels, res_labels, id2tag):
        no_covered_index_res = self.remove_cover_index(np.array(index_res).T)
        for i in no_covered_index_res:
