...
class Lstm_Crf_Entity_Extraction(object):
    ...
    def train(self):
        ...
        eu = Evaluation_Utils()
        # 加载模型
        graph,word_ids,labels,sequence_lengths,dropout_pl,logits,transition_params,loss,train_op,global_add = make_graph(param)
        # 制作训练集和测试集
        train_seq_data = self.dp.parse_data(self.dp.train_data, self.dp.word2id, self.dp.tag2id)
        test_seq_data = self.dp.parse_data(self.dp.test_data, self.dp.word2id, self.dp.tag2id)
        # 训练模型
                ...
                for epoch in range(param.num_train_epochs):
                    ...
                    for seqs_, labels_, seq_len_ in self.dp.get_batch(train_seq_data, param.batch_size,
                        ...
                        _, l, global_nums, logits_, transition_params_ = sess.run(
                            [train_op, loss, global_add, logits, transition_params], {
                                word_ids: seqs_,
                                labels: labels_,
...
if __name__ == '__main__':
    ...
    # 加载已处理好的数据，开启训练
    # LCEE.data_process_reload()
    # LCEE.train()

    # 根据训练好的模型参数，预测整个测试集
    # LCEE.data_process_reload()
    # LCEE.make_all_predict(param_save_path,optional_reload_path)
