...
class ARCii_Text_Matching(object):
    ...
    def data_process_init(self):
        ...
        :return:
        """
        embedding_file = self.dp.config.get('DATA_PROCESS', 'embedding_file')
        vocab_file = self.dp.config.get('DATA_PROCESS', 'vocab_file')
        embed_size = self.dp.config.getint('DATA_PROCESS', 'embed_size')
        feature_selection_name = self.dp.config.get('DATA_PROCESS', 'feature_selection_name')
        self.dp.init(embedding_file, vocab_file, embed_size,feature_selection_name)
    ...
    def train(self):
        ...
        # 加载模型
        graph,left_input,right_input,labels,dropout_pl,loss,train_op,true,pred,accuracy,global_add = make_graph(param)
        # 制作训练集和测试集
        train_seq_data = self.dp.parse_data(self.dp.train_data, self.dp.word2id,  param.max_seq_length)
        test_seq_data = self.dp.parse_data(self.dp.test_data, self.dp.word2id, param.max_seq_length)
        # 训练模型
        with tf.Session(graph=graph) as sess:
            if param.is_training:
                ...
                for epoch in range(param.num_train_epochs):
                    ...
                    all_acc = 0
                    count_loss = 0
                    for res_seq1, res_seq2, res_labels in self.dp.get_batch(train_seq_data, param.batch_size, param.shuffle):
                        _, l, acc_, p, global_nums = sess.run([train_op, loss, accuracy, pred, global_add],
                                                              feed_dict={
                                                                  left_input: res_seq1,
                                                                  right_input: res_seq2,
                                                                  labels: res_labels,
                        ...
                        if global_nums % param.evaluation_per_step == 0:
                            print('-----------------valudation---------------')
                            res_seq1, res_seq2, res_labels = next(self.dp.get_batch(test_seq_data, param.batch_size, param.shuffle))
                            ...
                            print(classification_report(res_labels, p, target_names=target_names))
                            print('-----------------valudation---------------')
                    for res_seq1, res_seq2, res_labels in self.dp.get_batch(test_seq_data, param.batch_size, param.shuffle):
                        l, acc_, p = sess.run([loss, accuracy, pred],
                                              feed_dict={
                                                  left_input: res_seq1,
                                                  right_input: res_seq2,
    ...
    def make_all_predict(self,param_save_path,optional_reload_path=""):
        ...
        # 加载模型
        graph,left_input,right_input,labels,dropout_pl,loss,train_op,true,pred,accuracy,global_add = make_graph(
            param)
        # 制作训练集和测试集
        test_seq_data = self.dp.parse_data(self.dp.test_data, self.dp.word2id, param.max_seq_length)
    ...
    def parse_words(self, words):
        ...
        for sentence1, sentence2 in words:
            sentence1 = self.dp.tokenizer.cut(sentence1)
            sentence1 = [i for i in sentence1 if i not in self.dp.stopwords]
            sentence2 = self.dp.tokenizer.cut(sentence2)
            sentence2 = [i for i in sentence2 if i not in self.dp.stopwords]
    ...
    def predict(self,words,param_save_path,id2tag,optional_reload_path=""):
        ...
        with tf.Session(graph=graph) as sess:
            ...
            for res_seq1, res_seq2, res_labels in self.dp.get_batch(test_data, param.batch_size, param.shuffle):
                l, acc_, p = sess.run([loss, accuracy, pred], feed_dict={
                    left_input: res_seq1,
                    right_input: res_seq2,
                    labels: res_labels,
                    dropout_pl: 1.0
...
if __name__ == '__main__':
    ...
    param_save_path = './param_dir/param_2021_03_29_13_46.pkl'
    # optional_reload_path = ""
    optional_reload_path = "model_16_-_0.4756.ckpt"
    ...
    ATM = ARCii_Text_Matching(cfg_path)

    # 初始化数据，开启训练
    # ATM.data_process_init()
    # ATM.train()

    # 加载已处理好的数据，开启训练
    # ATM.data_process_reload()
    # ATM.train()

    # 根据训练好的模型参数，预测整个测试集
    ATM.data_process_reload()
    ATM.make_all_predict(param_save_path,optional_reload_path)

    # 根据训练好的模型参数，预测样本
    # ATM.data_process_reload()
    # pred_dict = ATM.predict(words,param_save_path,ATM.dp.id2tag,optional_reload_path)
    # print(pred_dict)
