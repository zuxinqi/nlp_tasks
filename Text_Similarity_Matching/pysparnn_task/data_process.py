...
class Date_Process(Base_Process):
    ...
    def init(self):
        ...
        self.tv, self.cp, self.word_list = self.data_process(self.raw_data, self.tokenizer, self.stopwords)
    ...
    def split_words(self, data, tokenizer, stopwords):
        ...
        for line in data:
            ...
            word_list.append(line.strip())
            cut_word_list.append(" ".join(seg_list))
    ...
    def data_process(self,data,tokenizer,stopwords):
        ...
        tv = TfidfVectorizer()
        features_vec = tv.fit_transform(cut_word_list)
        cp = ci.MultiClusterIndex(features_vec, word_list)
        return tv, cp, word_list

    def save_model(self,save_path):
        with open(save_path, 'wb') as f:
            pickle.dump((self.tv, self.cp, self.word_list), f)

    def load_model(self,load_path):
        with open(load_path, 'rb') as f:
            self.tv, self.cp, self.word_list = pickle.load(f)
