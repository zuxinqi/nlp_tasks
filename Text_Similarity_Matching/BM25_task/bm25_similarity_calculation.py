# -*- coding:utf-8 -*-
import os
from data_process_for_custom import Date_Process

os.environ["CUDA_VISIBLE_DEVICES"] = "0"


class BM25_Similarity_Calculation(object):
    ...
    def predict(self,words, return_quantity=10, threshold=0):
        ...
        similarities = self.dp.bm25.get_scores(cut_words)
...
if __name__ == '__main__':
    ...
    BSC = BM25_Similarity_Calculation(cfg_path)

    # 初始化数据，开启训练
    # BSC.data_process_init()
    # result_similarity = BSC.predict(words)
    # for sentence,similarity in result_similarity:
    #     print(sentence+"  "+str(similarity))

    # 加载已处理好的数据，开启训练
    BSC.data_process_reload()
    result_similarity = BSC.predict(words)
    for sentence,similarity in result_similarity:
        print(sentence+"  "+str(similarity))
