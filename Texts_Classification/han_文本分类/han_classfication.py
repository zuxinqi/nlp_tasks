...
class Han_Classfication(object):
    ...
    def train(self):
        ...
        # 加载模型
        graph,input_x,input_y,dropout_keep_prob,pred,loss,true_,train_op,acc,global_add = make_graph(param)
        # 制作训练集和测试集
...
if __name__ == '__main__':
    ...
    HC = Han_Classfication(cfg_path)

    # 初始化数据，开启训练
    # HC.data_process_init()
    # HC.train()

    # 加载已处理好的数据，开启训练
    HC.data_process_reload()
    HC.train()

    # 根据训练好的模型参数，预测整个测试集
    # HC.data_process_reload()
    # HC.make_all_predict(param_save_path,optional_reload_path)

    # 根据训练好的模型参数，预测样本
    # HC.data_process_reload()
    # predicted = HC.predict(words,param_save_path,optional_reload_path)
    # print(predicted)
