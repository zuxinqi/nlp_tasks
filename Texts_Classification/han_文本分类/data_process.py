...
class Date_Process(Base_Process):
    ...
    def parse_config(self,config_path):
        ...
        self.param.num_sentences = self.config.getint('MODEL', 'num_sentences')
    ...
    def read_data(self):
        with open(self.file_path, 'rb') as f:
            self.raw_train_data, self.raw_test_data = pickle.load(f)
    ...
    def data_process(self):
        ...
        :return:
        """
        all_tag = ["其他", "减费降税", "项目申报"]
        self.tag2id = {all_tag[i]: i for i in range(len(all_tag))}
        self.id2tag = {v: k for k, v in self.tag2id.items()}

        for id_, task_id, title, content, split_content_data, tag in self.raw_train_data:
            seg_list = self.tokenizer.cut(content)
            seg_list = [i for i in seg_list if i not in self.stopwords]
            if tag != "减费降税" and tag != "项目申报":
                tag = "其他"
            self.train_data.append([seg_list, tag])

        for id_, task_id, title, content, split_content_data, tag in self.raw_test_data:
            seg_list = self.tokenizer.cut(content)
            seg_list = [i for i in seg_list if i not in self.stopwords]
            if tag != "减费降税" and tag != "项目申报":
                tag = "其他"
            self.test_data.append([seg_list, tag])
