...
class RCNN_Classfication(object):
...
if __name__ == '__main__':
    ...
    RCNN = RCNN_Classfication(cfg_path)

    # 初始化数据，开启训练
    # RCNN.data_process_init()
    # RCNN.train()

    # 加载已处理好的数据，开启训练
    RCNN.data_process_reload()
    RCNN.train()

    # 根据训练好的模型参数，预测整个测试集
    # RCNN.data_process_reload()
    # RCNN.make_all_predict(param_save_path,optional_reload_path)
