...
def make_graph(param):
    ...
    with graph.as_default():
        ...
        with tf.name_scope('rnn'):
            ...
            (output_fw, output_bw), states = tf.nn.bidirectional_dynamic_rnn(rnn_cell_fw, rnn_cell_bw, embedded_wrods,
                                                              sequence_length=input_seq, dtype=tf.float32)


        last_layers = tf.concat([output_fw, embedded_wrods, output_bw], axis=2, name="last")
        # layer1 = tf.layers.dense(last_layers, param.hidden_num, activation=tf.nn.relu)
        fc_pool = tf.reduce_max(last_layers, axis=1)

        logits_ = tf.layers.dense(fc_pool, param.num_classes)
