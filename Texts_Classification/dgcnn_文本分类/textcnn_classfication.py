...
class TextCNN_Classfication(object):
    ...
    def train(self):
        ...
        # 加载模型
        graph, input_x, input_y, dropout_keep_prob, train_op, loss, pred, true_, acc, global_add = make_graph(param)
        # 制作训练集和测试集
        ...
        with tf.Session(graph=graph) as sess:
            if param.is_training:
                ...
                sess.run(init)
                # saver.restore(sess, "./checkpoints/checkpoint_2021_02_08_11_05/model_39___0.5896.ckpt")
    ...
    def make_all_predict(self,param_save_path,optional_reload_path=""):
        ...
        param.shuffle = False
        # 加载模型
        graph, input_x, input_y, dropout_keep_prob, train_op, loss, pred, true_, acc, global_add = make_graph(
            param)
        # 获取测试数据
        test_seq_data = self.dp.parse_data(self.dp.test_data, self.dp.word2id, self.dp.tag2id, param.max_seq_length)
        # 获取最小loss的ckpt文件路径
...
if __name__ == '__main__':
    ...
    # optional_reload_path = "model_13_-_0.3667.ckpt"
    ...
    TCC = TextCNN_Classfication(cfg_path)

    # 初始化数据，开启训练
    # TCC.data_process_init()
    # TCC.train()

    # 加载已处理好的数据，开启训练
    TCC.data_process_reload()
    TCC.train()

    # 根据训练好的模型参数，预测整个测试集
    # TCC.data_process_reload()
    # TCC.make_all_predict(param_save_path,optional_reload_path)

    # 根据训练好的模型参数，预测样本
    # TCC.data_process_reload()
    # predicted = TCC.predict(words,param_save_path,optional_reload_path)
    # print(predicted)
