...
class Date_Process(Base_Process):
    ...
    def parse_data(self, data, bert_tokenizer, tag2id, max_length):
        ...
        for words, tag in data:
            words.insert(0, "[CLS]")
            words.append("[SEP]")
            sents = self.convert_tokens_to_ids(bert_tokenizer, words, self.do_lower_case)
            task_ids = [1] * len(sents)
            segment_ids = [0] * len(sents)
            tag = tag2id[tag]

            if len(sents) < max_length:
                pad = [0] * (max_length - len(sents))
                sents = sents + pad
            else:
            ...
            all_seq.append((sents, task_ids, segment_ids, tag))
        return all_seq
