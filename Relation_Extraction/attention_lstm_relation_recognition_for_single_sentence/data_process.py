...
class Date_Process(Base_Process):
    ...
    def parse_data(self, data, word2id, tag2id, max_seq_length):
        ...
        :param tag2id: 字典，类似{"tag1":1:0,"tag2":1,"tag3":2......}
    ...
    def get_batch(self, data, batch_size, shuffle=False):
        ...
        for i in range(len(data) // batch_size):
            ...
            seqs, e1_list, e2_list, labels = [], [], [], []
            for (sent_, e1_, e2_, tag_) in data_size:
                seqs.append(sent_)
                e1_list.append(e1_)
                e2_list.append(e2_)
                labels.append(tag_)
            yield np.array(seqs), np.array(e1_list), np.array(e2_list), np.array(labels),batch_size
        ...
        if remainder != 0:
            ...
            for (sent_, e1_, e2_, tag_) in data_size:
                ...
                e2_list.append(e2_)
                labels.append(tag_)
            yield np.array(seqs), np.array(e1_list), np.array(e2_list), np.array(labels), remainder
