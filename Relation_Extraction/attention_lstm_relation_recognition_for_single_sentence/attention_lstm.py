...
class Attention_Lstm_Relation_Recognition(object):
    ...
    def train(self):
        ...
        # 加载模型
        graph,input_word,input_pos1,input_pos2,input_y,input_batch_size,predictions,true_,acc,final_loss,train_op,global_add = make_graph(param)
        # 制作训练集和测试集
        ...
        with tf.Session(graph=graph) as sess:
            if param.is_training:
                ...
                for epoch in range(param.num_train_epochs):
                    ...
                    all_acc = 0
                    count_loss = 0
                    for res_seq, e1_list, e2_list, res_labels,batch_size_ in self.dp.get_batch(train_seq_data, param.batch_size,
                                                                                                 param.shuffle):
                        ...
                        if global_nums % param.evaluation_per_step == 0:
                            print('-----------------valudation---------------')
                            res_seq, e1_list, e2_list, res_labels, batch_size_ = next(self.dp.get_batch(test_seq_data, param.batch_size, param.shuffle))
                            l, acc_, t, p = sess.run(
                            ...
                            print(classification_report(t, p, target_names=target_names))
                            print('-----------------valudation---------------')
                    for res_seq, e1_list, e2_list, res_labels, batch_size_ in self.dp.get_batch(test_seq_data, param.batch_size, param.shuffle):
                        l, acc_, t, p = sess.run(
                            [final_loss, acc, true_, predictions],
                            feed_dict={
                                input_word: res_seq,
                ...
                m = 0
                all_loss = 0
                for res_seq, e1_list, e2_list, res_labels,batch_size_ in self.dp.get_batch(test_seq_data,
                                                                                             param.batch_size,
                                                                                             param.shuffle):
                    l, acc_, t, p = sess.run(
                        [final_loss, acc, true_, predictions],
...
if __name__ == '__main__':
    ...
    param_save_path = './param_dir/param_2021_03_16_14_11.pkl'
    # optional_reload_path = ""
    optional_reload_path = "model_9___0.1691.ckpt"
    ...
    # ALRR.data_process_reload()
    # file_path = "/root/zxq/all_models/Relation_Extraction/entity_extraction_bert_lstm_crf/entity_list.pkl"
    # pred_dict = ALRR.predict(file_path,param_save_path,ALRR.dp.id2tag,optional_reload_path)
