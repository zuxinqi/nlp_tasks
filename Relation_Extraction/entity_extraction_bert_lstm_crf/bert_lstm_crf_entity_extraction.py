import re,os,datetime,argparse,pickle
import numpy as np
...
class Bert_Lstm_Crf_Entity_Extraction(object):
    ...
    def train(self):
        ...
        eu = Evaluation_Utils()
        # 加载模型
        graph,input_ids, input_mask, segment_ids, label_ids, total_loss, logits, trans, pred_ids, train_op, global_add = make_graph(param)
        # 制作训练集和测试集
        train_seq_data = self.dp.parse_data(self.dp.train_data, self.dp.bert_tokenizer, self.dp.tag2id,
        ...
        with tf.Session(graph=graph) as sess:
            if param.is_training:
                ...
                for epoch in range(param.num_train_epochs):
                    ...
                    all_acc = 0
                    count_loss = 0
                    for seqs, masks,segments, labels in self.dp.get_batch(train_seq_data, param.batch_size, param.shuffle):
                        _, l, global_nums, logits_, transition_params_ = sess.run(
                            [train_op, total_loss, global_add, logits, trans], {
                                input_ids: seqs,
                                input_mask: masks,
                        ...
                        if global_nums % param.display_per_step == 0:
                            # 获取真实序列、标签长度。
                            label_list, pred_list = eu.make_mask(logits_, labels, masks, True,
                                                                               transition_params_)
                            ...
                            repo_res_dict = eu.calculate_report(label_list, pred_list, self.dp.id2tag)
                        ...
                        if global_nums % param.evaluation_per_step == 0:
                            print('-----------------valudation---------------')
                            seqs, masks,segments, labels = next(self.dp.get_batch(test_seq_data, param.batch_size, param.shuffle))
                            l, logits_, transition_params_ = sess.run([total_loss, logits, trans],
                                                                      feed_dict={
                                                                          input_ids: seqs,
                                                                          input_mask: masks,
                                                                          segment_ids: segments,
                                ...
                                print(
                                    '{}   Rec:{:6.2%}  Pre:{:6.2%}   f1:{:6.2%}  count:  {}'.format(key,
                                                                                                    ...
                                                                                                    repo_res_dict[key][
                                                                                                        3]))
                            print('-----------------valudation---------------')
                    for seqs, masks,segments, labels in self.dp.get_batch(test_seq_data, param.batch_size, param.shuffle):
                        l, logits_, transition_params_ = sess.run([total_loss, logits, trans],
                                                                  feed_dict={
                                                                      input_ids: seqs,
                                                                      input_mask: masks,
                ...
                repo_res_dict = eu.calculate_report(all_label_list, all_pred_list, self.dp.id2tag)
                for key in repo_res_dict.keys():
                    print(
    ...
    def make_all_predict(self,param_save_path,optional_reload_path=""):
        ...
        eu = Evaluation_Utils()
        # 加载模型
        graph, input_ids, input_mask, segment_ids, label_ids, total_loss, logits, trans, pred_ids, train_op, global_add = make_graph(
            param)
        # 获取测试数据
        test_seq_data = self.dp.parse_data(self.dp.test_data, self.dp.bert_tokenizer, self.dp.tag2id,
                                           param.max_seq_length)
    ...
    def predict(self,words,param_save_path,optional_reload_path=""):
        ...
        test_data = self.dp.parse_data(words_list, self.dp.bert_tokenizer, self.dp.tag2id,
                                           param.max_seq_length)
        # 获取最小loss的ckpt文件路径
        dirs = os.listdir(param.save_checkpoints_path)
        print(dirs)
        ...
        with tf.Session(graph=graph) as sess:
            ...
            if optional_reload_path == "":
                ...
                saver.restore(sess, param.save_checkpoints_path + optional_reload_path)
            for seqs, masks, segments, labels in self.dp.get_batch(test_data, param.batch_size, param.shuffle):
                l, logits_, transition_params_ = sess.run([total_loss, logits, trans],
                                                          feed_dict={
                                                              input_ids: seqs,
                                                              input_mask: masks,
                ...
                for one_token_, pred in zip(seqs, pred_list):
    ...
    # 对整个测试集进行测试
    def make_all_predict_for_complete_task(self,file_path,param_save_path,optional_reload_path=""):
        """
        对整个训练集进行测试，可改变参数文件
        :param param_save_path: 保存的参数文件路径
        :param optional_reload_path: 自选的加载的参数文件路径，如果为空字符串，则自动选择最低loss模型
        ...
        with open(file_path, 'rb') as f:
            raw_train_data, raw_test_data, relation2id, id2relation = pickle.load(f)
        # 获取测试数据
        # raw_test_data 和关系抽取的输入数据一致就可以
        test_data = self.dp.parse_raw_data(raw_test_data)
        test_seq_data = self.dp.parse_data(test_data, self.dp.bert_tokenizer, self.dp.tag2id,
                                           param.max_seq_length)
        ...
        with tf.Session(graph=graph) as sess:
            ...
            for seqs, masks, segments, labels in self.dp.get_batch(test_seq_data, param.batch_size, param.shuffle):
                ...
                label_list, pred_list = eu.make_mask(logits_, labels, masks, True,
                                                     transition_params_,"pred")
                all_pred_list+=pred_list
            for data, pred in zip(raw_test_data, all_pred_list):
                one_token_ = data["text"]
                tags = [self.dp.id2tag[data] for data in pred]
                # 减少一个数据长度，相当于把[CLS]去掉
                tags = tags[1:]
                res_dict = eu.transfrom2seq(tags)
                entity_dict, entity_list = eu.extract_entity(one_token_, res_dict)
                res_data.append([data,entity_dict,entity_list])
        return res_data
...
if __name__ == '__main__':
    ...
    param_save_path = './param_dir/param_2021_03_15_11_06.pkl'
    # optional_reload_path = ""
    optional_reload_path = "model_8_-_8.9224.ckpt"
    ...
    BLCEE = Bert_Lstm_Crf_Entity_Extraction(cfg_path)
    ...
    # 根据训练好的模型参数，预测样本
    # BLCEE.data_process_reload()
    # all_entity_dict, all_entity_list = BLCEE.predict(words,param_save_path,optional_reload_path)
    # for i, j in zip(all_entity_dict, all_entity_list):
    #     print(i)
    #     print(j)

    # 预测测试集，制作可以传入关系分类的数据集
    file_path = "../数据准备/lic2019_relation_extraction_data.pkl"
    BLCEE.data_process_reload()
    res_data = BLCEE.make_all_predict_for_complete_task(file_path,param_save_path,optional_reload_path)
