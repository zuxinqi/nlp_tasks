...
def E2EModel(param):
    bert_model = load_trained_model_from_checkpoint(param.bert_config_file, param.init_checkpoint, seq_len=None)
    ...
    gold_obj_heads_in = Input(shape=(None, param.num_rels))
    gold_obj_tails_in = Input(shape=(None, param.num_rels))
    ...
    pred_obj_heads = Dense(param.num_rels, activation='sigmoid')(tokens_feature)
    pred_obj_tails = Dense(param.num_rels, activation='sigmoid')(tokens_feature)
    ...
    hbt_model.compile(optimizer=Adam(param.learning_rate))
    hbt_model.summary()

    return subject_model, object_model, hbt_model

class Evaluate(Callback):
    def __init__(self, subject_model, object_model, tokenizer, id2rel, eval_data, save_weights_path, train_log, min_delta=1e-4, patience=7):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        with open(self.train_log, "a+", encoding="utf-8") as f:
            f.write('this number %d f1: %.4f, precision: %.4f, recall: %.4f, best f1: %.4f\n' % (epoch + 1,f1, precision, recall, self.best))
